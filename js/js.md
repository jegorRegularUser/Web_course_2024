...
```
# 8. ООП
JavaScript, в отличие от того же Java, является прототипно-ориентированным языком, что означает, что объекты создаются напрямую, а не с использованием классов. И изначально js отрицал саму идею ооп и продвигал функциональную парадигму:
``` javascript
// Литерал объекта
const person = {
    name: "John"
